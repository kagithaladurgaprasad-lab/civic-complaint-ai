...
@app.get("/")
def home():
    return {"message": "Civic Complaint RAG API is running"}
...
@app.post("/complaints", response_model=ComplaintAnalysisResponse)
def create_complaint(
    ...
    # Heavy ML imports are intentionally lazy-loaded here.
    from complaint_embeddings import store_text_complaint
    from complaint_pipeline import process_complaint

    # Save uploaded image
    filename = f"complaint_{current_user['user_id']}_{image.filename}"
    file_path = os.path.join(UPLOAD_DIR, filename)

    with open(file_path, "wb") as buffer:
        shutil.copyfileobj(image.file, buffer)

    # Run AI complaint pipeline
    ...
    if municipal_sla:
        municipal_sla_days = municipal_sla.get("sla_days")
        municipal_sla_service = municipal_sla.get("service")
        municipal_sla_department = municipal_sla.get("department")
        municipal_sla_source = municipal_sla.get("source")
    else:
    ...
    # Create complaint database record
    ...
    # Store complaint text embedding in Qdrant
...
def get_my_complaints(
    ...
    complaints = (
        db.query(Complaint)
        .filter(Complaint.user_id == current_user["user_id"])
...
@app.get("/admin/complaints/{complaint_id}/details")
